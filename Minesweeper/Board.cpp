	...
	for (int a = 0; a < COLUMNS; a++) {
		for (int b = 0; b < ROWS; b++) {
			if (getCell(a, b, cells)->getIsOpen() == 1) {
				...
				iconSprite.setPosition(static_cast<float>(CELL_SIZE * a), static_cast<float>(CELL_SIZE * b));
			...
			if (gameOver != 0 && getCell(a, b, cells)->getEffectTimer() < EFFECT_DURATION) {
				int effectSize = static_cast<int>(2 * round(0.5f * CELL_SIZE * ((EFFECT_DURATION - getCell(a, b, cells)->getEffectTimer()) / static_cast<float>(EFFECT_DURATION))));
				...
				if (getCell(a, b, cells)->updateEffectTimer() == 1) {
					if (a - 1 >= 0 && getCell(a - 1, b, cells)->getEffectTimer() == EFFECT_DURATION) {
						getCell(a - 1, b, cells)->setEffectTimer(static_cast<int>(effect_duration_distribution(randomEngine)));
					}

					if (b - 1 >= 0 && getCell(a, b - 1, cells)->getEffectTimer() == EFFECT_DURATION) {
						getCell(a, b - 1, cells)->setEffectTimer(static_cast<int>(effect_duration_distribution(randomEngine)));
					}

					if (1 + a < COLUMNS && getCell(1 + a, b, cells)->getEffectTimer() == EFFECT_DURATION) {
						getCell(1 + a, b, cells)->setEffectTimer(static_cast<int>(effect_duration_distribution(randomEngine)));
					}
...
void Board::openCell(int x, int y) {
	if (firstClick == 0) {
		std::uniform_int_distribution<unsigned short> xDistribution(0, COLUMNS - 1);
		std::uniform_int_distribution<unsigned short> yDistribution(0, ROWS - 1);

		firstClick = 1;

		for (unsigned short a = 0; a < MINES; a++) {
			int mineX = static_cast<int>(xDistribution(randomEngine));
			int mineY = static_cast<int>(yDistribution(randomEngine));
	...
	if (gameOver == 0 && getCell(x, y, cells)->getIsFlagged() == 0) {
		if (getCell(x, y, cells)->open(cells) == 1) {
			...
			unsigned short totalClosedCells = 0;
