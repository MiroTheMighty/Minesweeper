...
void startGame() {
	...
	emoji.setPosition(static_cast<float>(CELL_SIZE * (COLUMNS/2) - emojiTexture.getSize().y), -FONT_HEIGHT + 4);
	...
	while (window.isOpen()) {
		...
		if (board.getGameOver() == 0) {
			...
			minutes = static_cast<int>(totalSeconds.count()) / 60;
			seconds = static_cast<int>(totalSeconds.count()) % 60;
		}

		while (FRAME_DURATION <= lag) {
			sf::Vector2i pixelPos = sf::Mouse::getPosition(window);
			sf::Vector2f worldPos = window.mapPixelToCoords(pixelPos);
			int mouseCellX = std::clamp(static_cast<int>(worldPos.x / static_cast<float>(CELL_SIZE * SCREEN_RESIZE)), 0, COLUMNS - 1);
			int mouseCellY = std::clamp(static_cast<int>(worldPos.y / static_cast<float>(CELL_SIZE * SCREEN_RESIZE)), 0, ROWS - 1);
			...
			if (lag < FRAME_DURATION) {
				...
				if (board.effectOver() == 1) {
					if (board.getGameOver() == 1) {
						drawText(1, static_cast<unsigned short>(round(0.5f * (CELL_SIZE * COLUMNS - 8 * FONT_WIDTH))), static_cast<unsigned short>(round(0.5f * (CELL_SIZE * ROWS - FONT_HEIGHT))), "VICTORY!", window);
						drawText(1, static_cast<unsigned short>(round(0.5f * (CELL_SIZE * COLUMNS - 32 * FONT_WIDTH))), static_cast<unsigned short>(round(0.5f * (CELL_SIZE * ROWS + FONT_HEIGHT * 2))), "Press the enter key to play again!", window);
						if (updateHighS) {
						...
						drawText(1, static_cast<unsigned short>(round(0.5f * (CELL_SIZE * COLUMNS - 4 * FONT_WIDTH))), static_cast<unsigned short>(round(0.5f * (CELL_SIZE * ROWS - 2 * FONT_HEIGHT))), "GAME\n\nOVER", window);
						drawText(1, static_cast<unsigned short>(round(0.5f * (CELL_SIZE * COLUMNS - 32 * FONT_WIDTH))), static_cast<unsigned short>(round(0.5f * (CELL_SIZE * ROWS + FONT_HEIGHT * 2))), "Press the enter key to play again!", window);
						minutes = 0;
						seconds = 0;
						startTime = std::chrono::steady_clock::now();
